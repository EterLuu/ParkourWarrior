package ink.ziip.hammer.parkourwarrior.command;
...
public class MainCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (!sender.hasPermission("parkourwarrior." + args[0])) {
            sender.sendMessage("&6[Parkour Warrior] &b你没有权限。");
            return true;
        }
    ...
    public List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (args.length == 1) {
            ...
            returnList.removeIf(s -> !s.startsWith(args[0]) || !sender.hasPermission("parkourwarrior." + s));
            return returnList;
        }
